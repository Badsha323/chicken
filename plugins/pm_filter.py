...
async def next_page(bot, query):
    ...
    if settings['button']:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", callback_data=f'files#{file.file_id + GPLINKS_API }'
                ),
            ]
            for file in files
        ]
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{file.file_name}", callback_data=f'files#{file.file_id + GPLINKS_API}'
                ),
                InlineKeyboardButton(
                    text=f"{get_size(file.file_size)}",
                    callback_data=f'files_#{file.file_id + GPLINKS_API}',
                ),
            ]
            for file in files
        ]
